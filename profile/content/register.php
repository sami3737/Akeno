...
if(isset($_SESSION['username']) && isset($_SESSION['T2SteamID64']) && !empty($_SESSION['T2SteamID64']) && $_SESSION['T2SteamID64'] != '')
{
	echo 'You already have a web account.';
}else{
	?>
				...
				<div class="row">
					<form onsubmit="checkRegister();return false;" class="form">
						...
						<div class="form-group">
							<div class="row">
								<div id="secretcode" class="col-sm-6" style="margin: 0;">
									<label for="usernameInput">Username <span class="text-red">*</span></label>
									<input type="text" id="usernameInput" class="form-control" value="<?php echo (isset($_SESSION['steam']) ? $_SESSION['steam']['response']['players'][0]['realname'] : '')?>" pattern=".{3,16}" required="" title="" aria-describedby="ui-tooltip-0"/>
								</div>
							</div>
						</div>
...
function checkRegister()
{
	...
	jQuery.post('./checkregisterinputs.php',{username:username1 ,password:password1 ,password2:password2 ,email:email1 ,email2:email2,code:entrycaptcha1},
	function(json){
