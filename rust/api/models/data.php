...
class dataModel
{
	...
	{
		return SqlManager::NumRows("SELECT CharID FROM ".Config::$shardDB.".dbo._Char WHERE CharID = $CharID");
	}
	...
	public function getPlayerData($charID)
	{
		...
		*/
		...
		SELECT CH.CharName16, CH.RefObjID, CH.CurLevel, CH.Strength, CH.Intellect,
			...
			ISNULL((SELECT Status FROM ".Config::$CMS_DB.".dbo._Char_Status WHERE Charname = CH.CharName16 collate Latin1_General_CI_AS),'Offline') as Status, CH.LastLogout
		FROM ".Config::$shardDB.".dbo._Char AS CH
			INNER JOIN ".Config::$shardDB.".dbo._CharTrijob AS CJ ON CJ.CharID = CH.CharID
			INNER JOIN ".Config::$shardDB.".dbo._Inventory AS INV ON INV.CharID = CH.CharID 
			INNER JOIN ".Config::$shardDB.".dbo._Items AS IT ON IT.ID64 = INV.ItemID
			INNER JOIN ".Config::$shardDB.".dbo._Guild AS GU ON GU.ID = CH.GuildID
	...
	public function getCharItembySlot($charID,$slot)
	{
		...
		("
		SELECT * 
		FROM ".Config::$shardDB.".dbo._Inventory AS Inventory
			JOIN ".Config::$shardDB.".dbo._Items AS Items ON Items.ID64 = Inventory.ItemID
			LEFT JOIN ".Config::$shardDB.".dbo._BindingOptionWithItem AS BindingItem ON BindingItem.nItemDBID = Items.ID64 AND BindingItem.nOptValue > 0
			JOIN ".Config::$shardDB.".dbo._RefObjCommon AS RefObjCommon ON RefObjCommon.ID = ITEMS.RefItemID
			JOIN ".Config::$shardDB.".dbo._RefObjItem AS RefObjItem ON RefObjItem.ID = RefObjCommon.Link
	...
	public function getCharAvatars($charID,$slot)
	{
		...
		("
		SELECT * 
		FROM ".Config::$shardDB.".dbo._InventoryForAvatar AS Inventory
			JOIN ".Config::$shardDB.".dbo._Items AS Items ON Items.ID64 = Inventory.ItemID
			LEFT JOIN ".Config::$shardDB.".dbo._BindingOptionWithItem AS BindingItem ON BindingItem.nItemDBID = Items.ID64 AND BindingItem.nOptValue > 0
			JOIN ".Config::$shardDB.".dbo._RefObjCommon AS RefObjCommon ON RefObjCommon.ID = ITEMS.RefItemID
			JOIN ".Config::$shardDB.".dbo._RefObjItem AS RefObjItem ON RefObjItem.ID = RefObjCommon.Link
	...
	private function printitemDetials($result)
	{
		...
		if(isset($result['itemInfo']['TypeID2']))	
		{
			...
			$details['name'] = SQLManager::ReadSingle("SELECT Name FROM ".Config::$CMS_DB.".dbo._Web_ItemName WHERE CodeName128 like '%".$result['itemInfo']['NameStrID128']."'");
			return $details;
		}
...
//echo "<span style='color:#50cecd;font-weight: bold;'>Immortal (".$Value."Time/times)</span><br>";
	...
	public function getitemImage($result)
	{
		...
		$icon = "./img/media/icon/".$icon;
		
		if(!file_exists($icon))
		{
			$icon = "./img/media/icon/icon_default.png";
		}
		return $icon;
	}
	public function getItemInfo($result)
	{
		...
		$data['itemName'] = SQLManager::ReadSingle("SELECT Name FROM ".Config::$CMS_DB.".dbo._Web_ItemName WHERE CodeName128 like '%".$result['NameStrID128']."'");
    ...
    protected function getBluesStats($aItem, &$aSpecialInfo)
    {
        ...
        for ($i = 1; $i <= $aItem['MagParamNum']; $i++) {
            if (isset($aItem['MagParam' . $i]) && $aItem['MagParam' . $i] > 1) {
				...
				$level = SQLManager::ReadSingle("SELECT MLevel FROM ".Config::$shardDB.".dbo._RefMagicOpt WHERE ID = $MAGID");
				$name = SQLManager::ReadSingle("SELECT MOptName128 FROM ".Config::$shardDB.".dbo._RefMagicOpt WHERE ID = $MAGID");
